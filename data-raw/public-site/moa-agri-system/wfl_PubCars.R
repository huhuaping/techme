## code to prepare `wfl_useData_PubCars` dataset goes here
...
tbl_dir <- tribble(
  ~case, ~media, ~final,
  "PubCars", "data-raw/public-site/moa-agri-system/", c("xlsx/")
)
View(tbl_dir)
...
prefix_add <- "^list-industry.+?2011.+?.xlsx$" # default is NULL, other value may be "amount", "funds", only used when mode is "add_onex", "add_one", "edited_one"
pattern_sel <- choose.filePattern(
  year = c(2011), # may have length 1 or 2
...
find_result <- wfl.findFiles(
  ...
  dir.case = "PubCars", # the case name of the target directory
...
tbl_tidy <- tbl_read %>%
  # tidy the institution column
  mutate(
    researcher_inst = str_replace_all(researcher_inst, "\n", ""),
    researcher_inst = trimws(researcher_inst),
    institution_industry = str_replace_all(institution_industry, "\n", ""),
    institution_industry = trimws(institution_industry)
  ) %>%
  # obtain column of institution
  # which the value come from the column of `researcher_inst` and the `institution_industry`
  mutate(
    institution = ifelse(
      is.na(researcher_inst),
      institution_industry,
      researcher_inst
    )
  ) %>%
  # if there are multiple institutions, use the first one
  mutate(
    institution = ifelse(str_detect(institution, "ã€?),
...
if (any(!is.na(tbl_province$province))) {
  wfl.writeXlsx(
    ...
    year_target = 2011, # change here
    prefix_label = "tidy-list-industry"
  )
...
wfl.useData(
  ...
  character.cols = c(
    "chairman_industry", "area_num_eng", "func_num",
    "func_name", "institution_industry", "func_inst",
    "func_director"
  )
)
