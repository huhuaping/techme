...
tbl_dir <- tribble(
  ~case, ~media, ~final,
  "PubConvergence", "data-raw/public-site/moa-industry-convergence/", c("xlsx/")
)
View(tbl_dir)
...
prefix_add <- "^park-setup.+?2025.+?.xlsx$" # default is NULL, other value may be "amount", "funds", only used when mode is "add_onex", "add_one", "edited_one"
pattern_sel <- choose.filePattern(
  year = c(2025), # may have length 1 or 2
...
find_result <- wfl.findFiles(
  ...
  dir.case = "PubConvergence", # the case name of the target directory
...
dir_tidy <- dir_tar # just the raw directory
file_ptn <- "town-setup-.+?\\.xlsx$"
