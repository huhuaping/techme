...
find_result <- wfl.findFiles(
    ...
    dir.case = "budget", # the case name of the target directory
...
log_xlsx <- tibble::tribble(
    ...
    "agri_prod", 2, "åœ°åŒº", "data-raw/rural-yearbook/part03-agri-produce/02-fertilizer/raw-2022-2023.xlsx"
)
...
## Note:
##   - there may be useful english words in the variables names, such as "RDç»è´¹"
...
if (is_select) {
    df_tidy <- df_tidy %>%
        filter(vars %in% c("åœ°æ–¹ä¸€èˆ¬å…¬å…±é¢„ç®—æ”¯å‡?, "æ•™è‚²æ”¯å‡º", "ç§‘å­¦æŠ€æœ¯æ”¯å‡?, "å†œæ—æ°´æ”¯å‡?))
}
