...
## setting 0: unprotect all raw files
## only use for historical data set
# list_files <- list.files(path = "data-raw/agrimachine-yearbook/machine-service/", pattern = "\\.xls$", full.names = TRUE)
# for (file in list_files[20]) {
#     wfl.Xls2Xlsx(file_path = file, sheet_drop = c("CNKI"))
# }
...
find_result <- wfl.findFiles(
    ...
    dir.case = "machine_service", # the case name of the target directory
...
## check the xlsx file and may need to add "Âú∞Âå∫" to table column
## step 1: open the xlsx file
## step 2Ôº?check the first column name
## step 3Ôºöcheck and save the log detail for yearly update
##         log information:
##         - dir.case
##         - i.final
##         - mark
...
log_xlsx <- tibble::tribble(
    ...
    "agri_prod", 2, "Âú∞Âå∫", "data-raw/rural-yearbook/part03-agri-produce/02-fertilizer/raw-2022-2023.xlsx"
)



# Workflow: unpivot xlsx file ----
## setting 1Ôº?whether drop columns and specify the header mode.
cols_drop <- c(2) # drop the second column, as it is the english region column
## cols_drop <- NULL

## setting 2Ôº?choose header mode
...
## setting 3Ôº?specify the regex pattern for table identifier
# pattern_table <- "^Âú?*Âå? # not to use "Áª≠Ë°®" !

## setting 4Ôº?specify the target block list
...
df_out <- wfl.unpivotXlsx(
    ...
    pattern.table = "^Âú?*Âå?, # default is "^Âú?*Âå?
    reg_start = "^Âú?*Âå?, # getRange() argument
    reg_end = "^Êñ?*Áñ?, # getRange() argument
    unit_pattern = "Âçï‰Ωç:|Âçï‰ΩçÔº? # getInfo() argument
)
...
## Note:
##   - there may be useful english words in the variables names, such as "RDÁªèË¥π"
...
if (is_english) {
    df_tidy <- df_tidy %>%
        mutate(
            vars = str_replace_all(vars, "[A-Z][a-z]+", ""),
        ) %>%
        filter(!is.na(value)) # pay attention to the NA values
}
