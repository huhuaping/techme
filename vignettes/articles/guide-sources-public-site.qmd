...
当前维护：2024年6月已经迁移到`web-scrape/proj/torch-innocom`项目下）。

历史维护：在当前项目文件下进行爬取和数据管理。
...
```{r}
dir_tar <- here::here("topic/public-site/most-jcs-open-share/")
fs::dir_tree(dir_tar)
```
...
```{r}
dir_tar <- here("topic/public-site/moa-agri-system")
fs::dir_tree(dir_tar)
```
...
- 查询机构的省份信息。如有未知机构省份信息，则要进行Tianyan查询补充流程！
...
```{r}
dir_tar <- here("topic/public-site/moa-agrimodern-zone")
fs::dir_tree(dir_tar)
```
...
```{r}
dir_tar <- here("topic/public-site/moa-freshkeep-county/")
fs::dir_tree(dir_tar)
```
...
```{r}
dir_tar <- here("topic/public-site/moa-rural-infobase/")
fs::dir_tree(dir_tar)
```
...
```{r}
dir_tar <- here("topic/public-site/moa-genetic-resource/")
fs::dir_tree(dir_tar)
```
