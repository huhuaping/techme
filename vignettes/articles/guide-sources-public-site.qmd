...
- 采用独立的Rstudio项目`github/scrape-agrifirm`进行爬取

当前项目：

- 数据抓取，迁移到`web-scrape/proj/moa-seed-firm`项目下）

```{r}
dir_tar <- "d:/github/web-scrape/proj/moa-seed-firm"
fs::dir_tree(dir_tar, recurse = FALSE)
```
...
```{r}
dir_tar <- here("topic/public-site/moa-seed-firm")
fs::dir_tree(dir_tar)
```
