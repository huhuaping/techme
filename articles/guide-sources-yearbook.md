...
（2）统计口径说明：论文国际引用`RDPaperInternational`数据集年度数据具有一年滞后性，例如《中国科技统计年鉴2025》一般应该收录展示2024年的统计数据，但是论文国际引用数据表展示的是2023年的数据。请注意对这个年份数据进行特例化处理。截止2026年08月27日，实际上已经更新了2025年的年鉴数据（年鉴中滞后列出了2023年度的论文国际引用情况数据），可以在年度报告中调用这个数据集，但是要对年份进行特殊处理。
